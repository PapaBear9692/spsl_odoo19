...
class ContractTemplateLine(models.Model):
    ...
    currency_id = fields.Many2one(
        "res.currency",
        related="contract_id.pricelist_id.currency_id",
    )
    ...
    # === Line-Level Recurrence Fields ===
    # These are computed from contract or kept local depending on line_recurrence
    ...
    def _compute_name(self):
        """Compute description from product."""
        for line in self:
            if line.product_id:
    ...
    def _compute_is_recurring_note(self):
        """Check if this is a recurring note."""
        for record in self:
    ...
    def _compute_price_unit(self):
        """Compute unit price from pricelist or specific price."""
        for line in self:
    ...
    def _inverse_price_unit(self):
        """Store price in specific_price when not automatic."""
    ...
    def _set_recurrence_field(self, field):
        """Sync recurrence field from contract or keep local depending on config."""
        for record in self:
            record[field] = (
                record[field]
