...
class ContractContract(models.Model):
    ...
    def action_contract_send(self):
        self.ensure_one()
        template = self.env.ref("contract_19.email_contract_template", False)
    ...
    def get_formview_id(self, access_uid=None):
        if self.contract_type == "sale":
            return self.env.ref("contract_19.contract_contract_customer_form_view").id
        else:
            return self.env.ref("contract_19.contract_contract_supplier_form_view").id

    def _set_start_contract_modification(self):
        subtype_id = self.env.ref("contract_19.mail_message_subtype_contract_modification")
        for record in self:
            ...
            if modification_ids_not_sent:
                if not self.env.context.get("skip_modification_mail"):
                    subtype_id = self.env["ir.model.data"]._xmlid_to_res_id(
                        "contract_19.mail_message_subtype_contract_modification"
                    )
                    template_id = self.env.ref(
                        "contract_19.mail_template_contract_modification"
                    )
                    record.message_post_with_source(
                        template_id,
                        subtype_id=subtype_id,
                        email_layout_xmlid="contract_19.template_contract_modification",
                    )
    ...
    def _invoice_followers(self, invoices):
        invoice_create_subtype = self.env.ref(
            "contract_19.mail_message_subtype_invoice_created"
        )
        for item in self:
