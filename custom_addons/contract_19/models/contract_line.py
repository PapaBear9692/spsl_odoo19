...
class ContractLine(models.Model):
    ...
    def get_view(self, view_id=None, view_type="form", **options):
        ...
        if view_type == "list" and default_contract_type == "purchase":
            view_id = self.env.ref("contract_19.contract_line_supplier_tree_view").id
        if view_type == "form":
            if default_contract_type == "purchase":
                view_id = self.env.ref("contract_19.contract_line_supplier_form_view").id
            elif default_contract_type == "sale":
                view_id = self.env.ref("contract_19.contract_line_customer_form_view").id
